...
public:
    ...
    unordered_map() : 
        node(4) {};
    friend Iterator;
    // // Создает новый unordered_map, являющийся глубокой копией other [O(n)]
    // // unordered_map<std::string, int>  map;
    // // map["something"] = 69;
    // // map["anything"] = 199;
    // // unordered_map<std::string, int> copied{map};
    ...
    // // Возвращает размер unordered_map (сколько элементов добавлено)
    ...
    // // Проверяет является ли unordered_map пустым
    ...
    // // unordered_map<int, std::string> c =
    // //   {
    // //       {1, "one" }, {2, "two" }, {3, "three"},
    // //       {4, "four"}, {5, "five"}, {6, "six"  }
    // //   };
